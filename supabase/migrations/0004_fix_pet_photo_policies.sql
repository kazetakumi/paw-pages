...
--
--      exists (select 1 from pets p where p.id::text = (storage.foldername(name))[1])
--
--    the unqualified `name` binds to the inner scope — `pets.name`, the
--    animal's name — not `storage.objects.name`, the object path. Postgres
...
--    `handler_owns_entries` in 0001 avoids this by writing `entries.pet_id`.
--
-- 2. `anon` cannot see `pets`. The public-read policy subqueries `pets`, whose
--    only policies are `to authenticated`. Under `set local role anon` that
--    subquery returns no rows, so the policy denied even with 1 fixed.
--
...
-- ---------------------------------------------------------------------------
-- Routed through public_pets rather than pets. That view is security definer
-- and granted to anon, so a visitor can actually evaluate it, and it already
-- carries `where is_public and archived_at is null` — which keeps the original
-- intent exactly: switch the page off or archive the pet and the image stops
-- resolving at the same instant, with no extra code.
--
-- It matches on photo_path rather than on the folder segment because
-- public_pets deliberately exposes no id. That is a tighter test, not a looser
