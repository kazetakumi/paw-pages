...
-- reads.
--
-- It hangs off `pawpages_entries` rather than getting a table of its own because a
...
-- `pawpages_public_entries` is deliberately left alone — a weight is not for visitors,
...
-- A number with no unit is not a weight. Mirrors `pawpages_archive_is_complete`
-- on `pawpages_pets`: the pair travels together or neither is there. Valid
-- immediately because both columns are null on every existing row.
alter table pawpages_entries
  add constraint pawpages_weight_is_complete check (
